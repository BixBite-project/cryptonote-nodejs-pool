...
function handleGetMarket(urlParts, response){
    ...
    let tickers = []
    for (let i of config.prices.pairs) {
    	tickers.push(config.symbol + '-' + i)
    }
    ...
    // Get market prices
    log('info', logSystem, 'Asking for new market prices at ' + exchange);
    if (cachedMarketData[exchange] && cachedMarketData[exchange].timestamp) {
        let delta = Math.round(Date.now() / 1000) - cachedMarketData[exchange].timestamp
        if (delta < 300) { // 5 min to till cache renew
            log('info', logSystem, 'Found cached market prices, expires in ' + (300 - delta) + ' seconds');
            response.end(JSON.stringify(cachedMarketData[exchange].data));
            return;
        }
    }
    ...
    market.get(exchange, tickers, function(data) {
        if (data) {
            log('info', logSystem, 'Renewing cache for market prices...');
            cachedMarketData[exchange] = {
